...
class AttractionSearchInput(ToolInput):
    ...
    query: Optional[str] = Field(default=None, description="Search query for specific attractions")
    category: Optional[str] = Field(default=None, description="Attraction category (e.g., 'tourist_attraction', 'museum', 'park')")
    radius_meters: Optional[int] = Field(default=5000, description="Search radius in meters (max 50000)")
    min_rating: Optional[float] = Field(default=None, description="Minimum rating filter (0-5)")
    max_results: Optional[int] = Field(default=10, description="Maximum number of results")
    include_photos: Optional[bool] = Field(default=True, description="Include photo URLs in results")
    price_levels: Optional[List[int]] = Field(default=None, description="Price level filter (0-4)")
...
class AttractionSearchTool(BaseTool):
    ...
    def __init__(self):
        metadata = ToolMetadata(
            ...
            tags=["attraction", "tourism", "activity", "sightseeing", "entertainment", "google_places"],
            timeout=30
        )
    ...
    async def _text_search(self, input_data: AttractionSearchInput) -> List[Attraction]:
        ...
        async with aiohttp.ClientSession() as session:
            # Build search query
            search_query = f"{input_data.query} in {input_data.location}" if input_data.query else input_data.location
            
            request_body = {
                "textQuery": search_query,
                "maxResultCount": min(input_data.max_results or 20, 20),  # Max 20 for text search
                "locationBias": {
                    "rectangle": {
            ...
            async with session.post(
                ...
                if response.status == 200:
                    ...
                    return [await self._parse_place(place, input_data.include_photos) for place in data.get("places", [])]
                else:
                    error_text = await response.text()
                    raise Exception(f"Google Places API error: {response.status} - {error_text}")

    async def _nearby_search(self, input_data: AttractionSearchInput) -> List[Attraction]:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            request_body = {
                ...
                "locationRestriction": {
                    "circle": {
                        ...
                        "radius": min(input_data.radius_meters or 5000, 50000)  # Max 50km
                    }
                }
            }
            ...
            if input_data.category:
                ...
                request_body["includedTypes"] = ["tourist_attraction", "museum", "park", "zoo", "aquarium", "amusement_park"]
    ...
    async def _parse_place(self, place_data: dict, include_photos: bool = True) -> Attraction:
        ...
        if include_photos and "photos" in place_data:
            photo_urls = await self._get_photo_urls(place_data["photos"][:3])  # Limit to 3 photos
        
        # Determine category and description
        category = self._determine_category(types)
        description = self._generate_description(name, category, formatted_address, rating, user_rating_count)
    ...
    def _generate_description(self, name: str, category: str, address: str, rating: float, review_count: int) -> str:
